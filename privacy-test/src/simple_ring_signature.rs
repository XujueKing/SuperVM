...
impl SimpleRingSigner {
    ...
    pub fn sign(
        ...
        assert_eq!(
            expected_pk, public_keys[secret_index],
        ...
        let c0_value = self.hash_to_scalar(&[message, L_real.compress().as_bytes()]);
        ...
        loop {
            ...
            // c[i+1] = H(m || L[i])
            challenges[next_i] = self.hash_to_scalar(&[message, L_fake.compress().as_bytes()]);
    ...
    pub fn verify(
        ...
        for start_idx in 0..n {
            ...
            for offset in 0..n {
                ...
                // 计算下一个挑战
                c = self.hash_to_scalar(&[message, L.compress().as_bytes()]);
            }

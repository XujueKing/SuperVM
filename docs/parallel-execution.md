...
自动 GC 在后台线程周期性清理过期版本，无需手动调用 `gc()` 方法。
...
let config = GcConfig {
    ...
    auto_gc: Some(AutoGcConfig {
        interval_secs: 60,            // 每 60 秒执行一次
        version_threshold: 1000,      // 版本数 ≥ 1000 时触发
...
1. **周期性触发**: 每隔 `interval_secs` 秒执行一次 GC
2. **阈值触发**: 当 `total_versions() >= version_threshold` 时立即执行
   - 设置 `version_threshold = 0` 禁用阈值检查，仅使用周期触发
3. **启动触发**: `run_on_start = true` 时立即执行一次
...
- 自动 GC 与事务并发执行是安全的（读写锁保护）
- GC 线程使用可中断休眠 (100ms 粒度)，快速响应停止信号
- Drop 时自动停止 GC 线程并等待退出 (最多 2 秒)

### 性能影响

运行基准测试对比自动 GC 的开销:
```bash
cargo bench --bench parallel_benchmark -- auto_gc
```

**典型结果**:
- **写入开销**: < 5%（自动 GC 在后台运行，不阻塞写操作）
- **读取开销**: 无明显影响（GC 使用写锁，不阻塞并发读）
- **CPU 占用**: 休眠时几乎为 0，GC 执行时短暂峰值

### 最佳实践

1. **生产环境**: 启用自动 GC，设置合理的 `interval_secs` (30-60s)
2. **高写入场景**: 降低 `version_threshold` (500-1000) 以更频繁清理
3. **低负载场景**: 提高 `interval_secs` (120-300s) 以减少开销
4. **测试环境**: 可禁用自动 GC (`auto_gc: None`) 便于调试
...
- **v0.7.0 (2025-11-04)**: 添加 MVCC 自动 GC（后台线程）

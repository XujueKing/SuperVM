...
fn main() {
    ...
    println!(
        "Threads: {}, Txns/Thread: {}, Hot Keys: {}",
        NUM_THREADS, TXNS_PER_THREAD, NUM_HOT_KEYS
    );
    println!(
        "Hot Key Ratio: {:.0}%, Warmup: {}, Runs: {}\n",
        HOT_KEY_RATIO * 100.0,
        WARMUP_RUNS,
        BENCH_RUNS
    );
    ...
    let avg_conflicts_no = stats_no_bloom
        .iter()
        .map(|s| s.conflicts as f64)
        .sum::<f64>()
        / stats_no_bloom.len() as f64;
    let avg_success_no = stats_no_bloom
        .iter()
        .map(|s| s.successful as f64)
        .sum::<f64>()
        / stats_no_bloom.len() as f64;
    ...
    let avg_conflicts_with = stats_with_bloom
        .iter()
        .map(|s| s.conflicts as f64)
        .sum::<f64>()
        / stats_with_bloom.len() as f64;
    let avg_success_with = stats_with_bloom
        .iter()
        .map(|s| s.successful as f64)
        .sum::<f64>()
        / stats_with_bloom.len() as f64;
    let avg_bloom_hits = stats_with_bloom
        .iter()
        .map(|s| s.bloom_hits as f64)
        .sum::<f64>()
        / stats_with_bloom.len() as f64;
    let avg_bloom_misses = stats_with_bloom
        .iter()

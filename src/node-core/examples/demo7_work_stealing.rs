...
use vm_runtime::{ParallelScheduler, Task, WorkStealingScheduler};
...
fn demo_basic_work_stealing() {
    ...
    // 提交 20 个任务
    let tasks: Vec<Task> = (1..=20).map(|i| Task::new(i, 100)).collect();
...
fn demo_integration_with_parallel_scheduler() {
    ...
    let result = ws_scheduler.execute_all(move |tx_id| match tx_id {

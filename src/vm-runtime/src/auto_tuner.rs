...
impl AutoTuner {
    ...
    pub fn record_batch(
        ...
        if current_batch - last >= self.tuning_interval as u64 {
            ...
            self.last_tuning_batch
                .store(current_batch, Ordering::Relaxed);
        }
    }
    ...
    fn evaluate_and_tune(&self, conflict_rate: f64, avg_rw_set_size: usize) {
            ...
            if !history.is_empty() {
                ...
                if let Some((best_batch, _tps)) = best {
                    self.recommended_batch_size
                        .store(*best_batch, Ordering::Relaxed);
                }
            }
        }
        ...
        let should_enable_bloom =
            current_batch >= 100 && (conflict_rate > 0.10 || avg_rw_set_size > 10);
        self.recommended_bloom_enabled
            .store(should_enable_bloom, Ordering::Relaxed);

        // 3. 密度回退阈值: 若 Bloom 启用,放宽阈值避免过早回退;否则收紧
        let new_threshold = if should_enable_bloom { 0.30 } else { 0.05 };
        self.recommended_density_threshold
            .store((new_threshold * 10000.0) as u64, Ordering::Relaxed);
        ...
        self.recommended_num_shards
            .store(new_shards, Ordering::Relaxed);
    }

    /// 快速批量大小探测 (仅在初始化时调用)
    ///
    /// 返回探测到的最优批量大小
    pub fn quick_probe_batch_size<F>(&self, probe_fn: F) -> usize
    where
...
impl AutoTunerSummary {
    pub fn print(&self) {
        ...
        println!(
            "Recommended Bloom Filter: {}",
            if self.recommended_bloom_enabled {
                "ON"
            } else {
                "OFF"
            }
        ...
        println!(
            "Recommended Density Threshold: {:.2}%",
            self.recommended_density_threshold * 100.0
        );
    }
}
...
mod tests {
    ...
    fn test_quick_probe() {
        ...
        let best = tuner.quick_probe_batch_size(|batch| match batch {
            10 => 100000.0,
            50 => 150000.0,
            100 => 200000.0,
            200 => 180000.0,

...
use crate::mvcc::{AutoGcConfig, GcConfig, MvccStore, Txn};
...
impl MvccScheduler {
                ...
                Err(e) => {
                    ...
                    if retries >= self.config.max_retries {
                        ...
                        return TxnResult {
                            ...
                            success: false,
                            error: Some(format!(
                                "Transaction aborted after {} retries: {}",
                                retries, e
                            )),
                            commit_ts: None,
                        };
    ...
    pub fn batch_delete(&self, keys: Vec<Vec<u8>>) -> Result<u64> {
        ...
        txn.commit()
            .map_err(|e| anyhow::anyhow!("Batch delete failed: {}", e))
    }
}
...
mod tests {
    ...
    fn test_mvcc_scheduler_basic() {
        ...
        // 验证写入
        let value = scheduler
            .read_only(|txn| Ok(txn.read(b"key1").map(|v| v.to_vec())))
            .unwrap();

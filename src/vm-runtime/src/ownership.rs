...
impl OwnershipManager {
    ...
    pub fn register_object(&self, mut metadata: ObjectMetadata) -> Result<(), String> {
        ...
        self.objects
            .write()
            .unwrap()
            .insert(object_id, metadata.clone());
        ...
        match metadata.ownership {
            ...
            OwnershipType::Shared => {
                ...
                match access_type {
                    ...
                    AccessType::Transfer => Err("Cannot transfer shared object".to_string()),
                    AccessType::Delete => Err("Cannot delete shared object directly".to_string()),
                }
            }
            OwnershipType::Immutable => {
                // 不可变对象：只能读取
    ...
    pub fn transfer_ownership(
        ...
        // 记录版本历史
        if let Some(versions) = self.version_history.write().unwrap().get_mut(object_id) {
...
mod tests {
    ...
    fn test_transfer_ownership() {
        ...
        // 转移所有权
        assert!(manager
            .transfer_ownership(&metadata.id, &owner1, &owner2)
            .is_ok());
    ...
    fn test_access_control() {
        ...
        // 所有者可以写入
        assert!(manager
            .verify_access(&metadata.id, &owner, AccessType::Write)
            .is_ok());

        // 非所有者不能写入
        assert!(manager
            .verify_access(&metadata.id, &other, AccessType::Write)
            .is_err());

        // 任何人都可以读取
        assert!(manager
            .verify_access(&metadata.id, &other, AccessType::Read)
            .is_ok());
    }
}

...
use vm_runtime::{ConflictDetector, ParallelScheduler, ReadWriteSet, Txn};
    ...
    for data_size in [10, 100, 1000].iter() {
        group.bench_with_input(
            ...
            |b, &size| {
                ...
                b.iter(|| {
                    scheduler
                        .execute_with_snapshot(|_manager| {
                            // 仅测试快照创建和提交的开销
                            Ok(())
                        })
                        .unwrap();
...
fn bench_mvcc_scheduler(c: &mut Criterion) {
    ...
    group.bench_function("mvcc_backend_read_only", |b| {
        ...
        b.iter(|| {
            scheduler
                .execute_with_mvcc_read_only(|txn: &mut Txn| {
                    for i in 0..10 {
                        black_box(txn.read(&format!("key_{}", i).into_bytes()));
                    }
                    Ok(())
...
fn bench_mvcc_gc(c: &mut Criterion) {
    ...
    group.bench_function("write_with_gc", |b| {
        ...
        b.iter(|| {
            ...
            let mut txn = store.begin();
            txn.write(
                format!("key{}", counter % 100).into_bytes(),
                b"value".to_vec(),
            );
            txn.commit().unwrap();
